...
namespace poselib
{
	...
	typedef struct CoordinateProps
	{
		...
		unsigned int poolIdx;//Key of the map that holds the iterators to the list entries of this data structure
	...
	private:
		...
		std::unordered_map<unsigned int, std::list<CoordinateProps>::iterator> correspondencePoolIdx;//Stores the iterator to every correspondencePool element. The key value is an continous index necessary for nanoflann
		unsigned int corrIdx;//Continous index starting with 0 counting all correspondences that were ever inserted into correspondencePool. The index is resetted when the KD tree is resetted.
	...
	private:
		...
		unsigned int getInliers(cv::Mat E, cv::Mat & mask, std::vector<double> & error);
		void clearHistoryAndPool();
		void checkInlRatThresholds();
		void filterNewCorrespondences(std::vector<cv::DMatch> matches, std::vector<cv::KeyPoint> kp1, std::vector<cv::KeyPoint> kp2, std::vector<double> error);
