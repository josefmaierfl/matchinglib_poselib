			   ...
			   std::string matcher_name, bool VFCrefine, bool ratioTest)
{
	...
	if(!matcher_name.compare("GMBSOF"))
	{
		...
		if(!ratioTest)
		{
			cout << "GMbSOF is executed by default with ratio test. If you want to change this, you have to edit the options of function AdvancedMatching and recompile." << endl;
			cout << "Performing GMbSOF with ratio test!" << endl;
		}
		err = AdvancedMatching( matcher, keypoints1, keypoints2, descriptors1, descriptors2, imgSi, finalMatches);//, true, 0.3, 3.5,  2, false, NULL, NULL, NULL, NULL, imgs[0], imgs[1]);
		if(err)
		{
 ...
 */
int getSubPixMatches(cv::Mat img1, cv::Mat img2, std::vector<cv::KeyPoint> *keypoints1, std::vector<cv::KeyPoint> *keypoints2, std::vector<bool> *inliers)
{
	cv::Mat results;
	...
	for(size_t i=0;i<keypoints1->size();i++)
	{
		...
		if(featuresize1 < 18) featuresize1 = 18;
		...
		if(((float)cvRound(featuresize_2)-featuresize_2) == 0.0) featuresize1 -= 1;
