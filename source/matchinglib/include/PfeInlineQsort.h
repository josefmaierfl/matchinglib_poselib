/* Adopted from qsort.h 2008-01-28 code by Michael Tokarev http://www.corpit.ru/mjt/qsort.html
 * for PfeLib by Oliver Zendel
 *
...
#define _QSORT_SWAP_NEG_DIFF(a, b, baseDiff, t) ((void)((t = *a), (*a = *b), (*b = t), (t = *(a-baseDiff)), (*(a-baseDiff) = *(b-baseDiff)), (*(b-baseDiff) = t)))
#define _QSORT_SWAP_POS_DIFF(a, b, baseDiff, t) ((void)((t = *a), (*a = *b), (*b = t), (t = *(a+baseDiff)), (*(a+baseDiff) = *(b+baseDiff)), (*(b+baseDiff) = t)))
...
#define Pfe32u_lt(a,b) ((*a)<(*b))
#define Pfe32u_gt(a,b) ((*a)>(*b))

#define INLINEQSORTSIMPLE(QSORT_TYPE,QSORT_BASE,QSORT_NELT,QSORT_LT)	\
{									\
...
#define INLINEQSORTREORDER(QSORT_TYPE,QSORT_BASE_CMP,QSORT_BASE_REORDER,QSORT_NELT,QSORT_LT)		\
{									\
  QSORT_TYPE *const _base     = (QSORT_TYPE *)(QSORT_BASE_CMP);			\
  QSORT_TYPE *const _base2    = (QSORT_TYPE *)(QSORT_BASE_REORDER);			\
  if(_base2<_base)                                                      \
       INLINEQSORTREORDERNEG(QSORT_TYPE,QSORT_BASE_CMP,QSORT_BASE_REORDER,QSORT_NELT,QSORT_LT)  \
  else					                                \
       INLINEQSORTREORDERPOS(QSORT_TYPE,QSORT_BASE_CMP,QSORT_BASE_REORDER,QSORT_NELT,QSORT_LT)  \
}

#define INLINEQSORTREORDERNEG(QSORT_TYPE,QSORT_BASE_CMP,QSORT_BASE_REORDER,QSORT_NELT,QSORT_LT)		\
{									\
  QSORT_TYPE *const _base     = (QSORT_TYPE *)(QSORT_BASE_CMP);			\
  QSORT_TYPE *const _base2    = (QSORT_TYPE *)(QSORT_BASE_REORDER);			\
  const signed   _baseDiff    = (signed)(_base-_base2);                           \
  const unsigned _elems = (QSORT_NELT);					\
  QSORT_TYPE _hold, _hold2;						\
									\
  /* Don't declare two variables of type QSORT_TYPE in a single		\
   * statement: eg `TYPE a, b;', in case if TYPE is a pointer,		\
   * expands to `type* a, b;' wich isn't what we want.			\
   ...
   if (_elems > _QSORT_MAX_THRESH) {					\
    ...
    while (_QSORT_STACK_NOT_EMPTY) {					\
      ...
      QSORT_TYPE *_mid = _lo + ((_hi - _lo) >> 1);			\
									\
      if (QSORT_LT (_mid, _lo))						\
        _QSORT_SWAP_NEG_DIFF (_mid, _lo, _baseDiff, _hold);			\
      if (QSORT_LT (_hi, _mid))	{					\
        _QSORT_SWAP_NEG_DIFF (_mid, _hi, _baseDiff, _hold);			\
        if (QSORT_LT (_mid, _lo))					\
          _QSORT_SWAP_NEG_DIFF (_mid, _lo, _baseDiff, _hold);			\
      } 								\
									\
      ...
      do {								\
        ...
        while (QSORT_LT (_mid, _right_ptr))				\
          --_right_ptr;							\
									\
        if (_left_ptr < _right_ptr) {					\
          _QSORT_SWAP_NEG_DIFF (_left_ptr, _right_ptr, _baseDiff, _hold);	\
          if (_mid == _left_ptr)					\
            _mid = _right_ptr;						\
          else if (_mid == _right_ptr)					\
            _mid = _left_ptr;						\
    ...
    for (_run_ptr = _tmp_ptr + 1; _run_ptr <= _thresh; ++_run_ptr)	\
      if (QSORT_LT (_run_ptr, _tmp_ptr))				\
        _tmp_ptr = _run_ptr;						\
									\
    if (_tmp_ptr != _base)						\
      _QSORT_SWAP_NEG_DIFF (_tmp_ptr, _base, _baseDiff, _hold);			\
									\
    /* Insertion sort, running from left-hand-side			\
     * up to right-hand-side.  */					\
									\
    ...
    while (++_run_ptr <= _end_ptr) {					\
      ...
      if (_tmp_ptr != _run_ptr) {					\
        ...
        while (--_trav >= _run_ptr) {					\
          QSORT_TYPE *_hi; QSORT_TYPE *_lo;				\
          _hold = *_trav;						\
	_hold2 = *(_trav - _baseDiff);                                  \
          for (_hi = _lo = _trav; --_lo >= _tmp_ptr; _hi = _lo)         \
          {                                                             \
            *_hi = *_lo;                                                \
            *(_hi - _baseDiff) = *(_lo - _baseDiff);                    \
          }                                                             \
          *_hi = _hold;	                                                \
          *(_hi - _baseDiff) = _hold2;                                  \
        }								\
      }									\
    }									\
  }									\
...
#define INLINEQSORTREORDERPOS(QSORT_TYPE,QSORT_BASE_CMP,QSORT_BASE_REORDER,QSORT_NELT,QSORT_LT)		\
{									\
  QSORT_TYPE *const _base     = (QSORT_TYPE *)(QSORT_BASE_CMP);			\
  QSORT_TYPE *const _base2    = (QSORT_TYPE *)(QSORT_BASE_REORDER);			\
  const signed   _baseDiff    = (signed)(_base2-_base);                           \
  const unsigned _elems = (QSORT_NELT);					\
  QSORT_TYPE _hold, _hold2;						\
									\
  /* Don't declare two variables of type QSORT_TYPE in a single		\
   ...
   if (_elems > _QSORT_MAX_THRESH) {					\
    ...
    while (_QSORT_STACK_NOT_EMPTY) {					\
      ...
      QSORT_TYPE *_mid = _lo + ((_hi - _lo) >> 1);			\
									\
      if (QSORT_LT (_mid, _lo))						\
        _QSORT_SWAP_POS_DIFF (_mid, _lo, _baseDiff, _hold);			\
      if (QSORT_LT (_hi, _mid))	{					\
        _QSORT_SWAP_POS_DIFF (_mid, _hi, _baseDiff, _hold);			\
        if (QSORT_LT (_mid, _lo))					\
          _QSORT_SWAP_POS_DIFF (_mid, _lo, _baseDiff, _hold);			\
      } 								\
									\
      ...
      do {								\
        ...
        while (QSORT_LT (_mid, _right_ptr))				\
          --_right_ptr;							\
									\
        if (_left_ptr < _right_ptr) {					\
          _QSORT_SWAP_POS_DIFF (_left_ptr, _right_ptr, _baseDiff, _hold);	\
          if (_mid == _left_ptr)					\
            _mid = _right_ptr;						\
          else if (_mid == _right_ptr)					\
            _mid = _left_ptr;						\
    ...
    for (_run_ptr = _tmp_ptr + 1; _run_ptr <= _thresh; ++_run_ptr)	\
      if (QSORT_LT (_run_ptr, _tmp_ptr))				\
        _tmp_ptr = _run_ptr;						\
									\
    if (_tmp_ptr != _base)						\
      _QSORT_SWAP_POS_DIFF (_tmp_ptr, _base, _baseDiff, _hold);			\
									\
    /* Insertion sort, running from left-hand-side			\
     * up to right-hand-side.  */					\
									\
