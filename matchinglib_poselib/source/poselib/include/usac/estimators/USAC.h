...
struct UsacResults {
	void reset() {
		...
		degen_inlier_count_rot   = 0;
		degen_inlier_count_trans = 0;
		degen_inlier_count_noMot = 0;
		...
		degen_inlier_flags_rot.clear();
		degen_inlier_flags_trans.clear();
		degen_inlier_flags_noMot.clear();
		degen_sample_.clear();
		sprt_delta_ = 0;
		sprt_epsilon_ = 0;
	}
	...
	unsigned int			  degen_inlier_count_rot;
	unsigned int			  degen_inlier_count_trans;
	unsigned int			  degen_inlier_count_noMot;
	std::vector<unsigned int> degen_inlier_flags_;
	std::vector<unsigned int> degen_inlier_flags_rot;
	std::vector<unsigned int> degen_inlier_flags_trans;
	std::vector<unsigned int> degen_inlier_flags_noMot;
...
class USAC
{
	...
	protected:
		...
		virtual inline bool		    generateRefinedModel(std::vector<unsigned int>& sample, const unsigned int numPoints, 
														 bool weighted = false, double* weights = nullptr) = 0;
	...
	protected:
		...
		unsigned int	usac_min_sample_size_degen;
	...
	protected:
		...
		inline void generateModelCompletionMinSample(unsigned int dataSize_degen, unsigned int dataSize, 
			unsigned int sampleSize_degen, unsigned int sampleSize_outl, std::vector<unsigned int>* sample);
...
void USAC<ProblemType>::initDataUSAC(const ConfigParams& cfg)
{
	...
	if (usac_test_degeneracy_)
	{
		...
		usac_results_.degen_inlier_flags_rot.resize(usac_num_data_points_, 0);
		usac_results_.degen_inlier_flags_trans.resize(usac_num_data_points_, 0);
		usac_results_.degen_inlier_flags_noMot.resize(usac_num_data_points_, 0);
...
bool USAC<ProblemType>::solve()
{
	...
	{
		usac_results_.sprt_delta_ = wald_test_history_->delta_;
		usac_results_.sprt_epsilon_ = wald_test_history_->epsilon_;
	}
...
// ============================================================================================
// generateModelCompletionMinSample: generate random sample uniformly distributed between 
// [0...dataSize_degen-1] and [dataSize_degen...dataSize]
// note that the sample vector needs to be properly sized before calling this function
// ============================================================================================
template <class ProblemType>
void USAC<ProblemType>::generateModelCompletionMinSample(unsigned int dataSize_degen, unsigned int dataSize, 
	unsigned int sampleSize_degen, unsigned int sampleSize_outl, std::vector<unsigned int>* sample)
{
	unsigned int count = 0;
	unsigned int index = 0;
...
void USAC<ProblemType>::generatePROSACMinSample(unsigned int hypCount, std::vector<unsigned int>* sample)
{
	...
	for (auto &i : *sample)
	{
		i = prosac_sorted_point_indices_[i];
	}
}
...
unsigned int USAC<ProblemType>::locallyOptimizeSolution(const unsigned int bestInliers)
{
	...
	auto *weights = new double[usac_num_data_points_];
...
unsigned int USAC<ProblemType>::updateSPRTStopping(unsigned int numInliers, unsigned int totPoints, TestHistorySPRT* testHistory)
{
	...
	for (unsigned int i = 0; i < usac_min_sample_size_; ++i)
	{
		n_inliers *= (double)(numInliers - i);
		n_pts *= (double)(totPoints - i);
	}
	double prob_good_model = n_inliers/n_pts;
	...
	while (current_test != nullptr)
	{
		...
		prob_reject_good_model = 1.0 /(exp( h * log(current_test->A_) ));
...
// ============================================================================================
template <class ProblemType> inline
double USAC<ProblemType>::computeExpSPRT(double new_Epsilon, double epsilon, double delta)
{
	double al, be, x0, x1, v0, v1, h;

	al = log(delta / epsilon);
	be = log( (1.0 - delta)/(1.0 - epsilon) );

	x0 = log( 1.0 / (1.0 - new_Epsilon) )/be;
	v0 = new_Epsilon * exp(x0 *al);
	x1 = log( (1.0 - 2.0 * v0) / (1.0 - new_Epsilon) )/be;
	v1 = new_Epsilon * exp(x1 * al) + (1.0 - new_Epsilon) * exp(x1 * be);
	h = x0 - (x0 - x1)/(1.0 + v0 - v1) * v0;
	return h;
}
...
{
	auto *new_test_history = new TestHistorySPRT;
