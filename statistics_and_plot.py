...
def getSymbolDescription(label):
    ...
    if label == 'R_diffAll_diff':
        ...
        return (replaceCSVLabels(label),
                ...
                '$e_{f\\left( R\\bm{t}\\right) }=\\left( e_{\\Sigma}-'
                '\\text{min}\\left( e_{\\Sigma}\\right)\\right)/r_{R\\bm{t}}$ with '
                '$e_{\\Sigma}=e_{f\\left( R\\right) }+e_{f\\left( \\bm{t}\\right) }$, '
                '$r_{R\\bm{t}}=\\text{max}\\left( e_{\\Sigma}\\right)-\\text{min}\\left( e_{\\Sigma}\\right)$, '
                '$e_{f\\left( R\\right) }=\\lvert \\mu_{\\Delta R_{\\Sigma}}\\rvert '
                '+\\sigma_{\\Delta R_{\\Sigma}}$, and '
        ...
        return (replaceCSVLabels(label),
                ...
                '\\left( \\hat{e}_{\\Sigma}-\\text{min}\\left( \\hat{e}_{\\Sigma}\\right)\\right)'
                '/r_{\\hat{R}\\hat{\\bm{t}}}$ with '
                '$\\hat{e}_{\\Sigma}=e_{f\\left( \\hat{R}\\right) }+e_{f\\left( \\hat{\\bm{t}}\\right) }$, '
                '$r_{\\hat{R}\\hat{\\bm{t}}}=\\text{max}\\left( \\hat{e}_{\\Sigma}\\right) '
                '-\\text{min}\\left( \\hat{e}_{\\Sigma}\\right)$, '
                '$e_{f\\left( \\hat{R}\\right) }=\\lvert \\mu_{\\Delta \\hat{R}_{\\Sigma}}\\rvert '
                '+\\sigma_{\\Delta \\hat{R}_{\\Sigma}}$, and '
        ...
        return (replaceCSVLabels(label),
                ...
                '\\left( \\Delta e_{\\Sigma}-\\text{min}\\left( \\Delta e_{\\Sigma}\\right)\\right)/r_{\\Delta e}$ '
                'of combined rotation $R$ and translation $\\bm{t}$ error differences '
                '$\\Delta^{2}R=\\Delta R_{i}-\\Delta R_{i-1}\\; \\forall i \\in \\left[ 1,\\; n_{I}\\; \\right]$ and '
                '$\\angle{\\Delta^{2} \\bm{t}}=\\angle{\\Delta \\bm{t}_{i}}-\\angle{\\Delta \\bm{t}_{i-1}}\\; '
                '\\forall i \\in \\left[ 1,\\; n_{I}\\; \\right]$ with '
                '$\\Delta e_{\\Sigma}=\\Delta^{2}R+\\angle{\\Delta^{2} \\bm{t}}$, '
                '$r_{\\Delta e}=\\text{max}\\left( \\Delta e_{\\Sigma}\\right)'
                '-\\text{min}\\left( \\Delta e_{\\Sigma}\\right)$, and number of stereo image pairs $n_{I}$.', True)
